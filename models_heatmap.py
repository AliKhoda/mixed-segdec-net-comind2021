...
class SegDecNet(nn.Module):
    def __init__(self, device, input_width, input_height, input_channels, neck_width=1024):
    ...
    def get_feat(self, input):
        ...
        seg_features_max = nn.MaxPool2d(kernel_size=8)(seg_mask)
        seg_features_avg = nn.AvgPool2d(kernel_size=8)(seg_mask)

        seg_features_max = self.glob_max_lr_multiplier_layer(seg_features_max, self.glob_max_lr_multiplier_mask)
        seg_features_avg = self.glob_avg_lr_multiplier_layer(seg_features_avg, self.glob_avg_lr_multiplier_mask)

        features_cat = torch.cat([features, features, seg_features_max, seg_features_avg], dim=1)
